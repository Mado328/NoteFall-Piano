"""
ColorTheme — единственный источник цветовых констант приложения.

Заменяет 20+ изменяемых глобальных переменных и хрупкую функцию
``_apply_colors(globals())`` — теперь тема передаётся через инъекцию
зависимостей, а не через глобальный модульный контекст.
"""
